...
def solve(G):
    ...
    for i in range(20):
        ...
        curr_pairwise_distance = min(average_pairwise_distance_fast(
            T), average_pairwise_distance_fast(Q))
        cur_graph = T if average_pairwise_distance_fast(
            T) < average_pairwise_distance_fast(Q) else Q
